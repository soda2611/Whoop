    ...
    if (e.key === "Enter") {
      ...
      try {
        ...
        if (entry.phonetics[1] && entry.phonetics[1].text) {
          html += `<span style="font-size: 15px"><b>${entry.phonetics[1].text}</b><br></span>`;
          html += `<br>`;
        } else if (entry.phonetics[0] && entry.phonetics[0].text) {
          html += `<span style="font-size: 15px"><b>${entry.phonetics[0].text}</b><br></span>`;
          html += `<br>`;
        }
        entry.meanings.forEach((meaning) => {
          ...
          meaning.definitions.forEach((def, idx) => {
            ...
            if (def.example)
              html += `<span style="color:#0e42d3">• ${def.example}</span><br><br>`;
            else html += `<br>`;
          });
          if (meaning.synonyms && meaning.synonyms.length > 0) {
            html += `<b>Synonyms:</b> ${meaning.synonyms.join(", ")}<br><br>`;
          }
          if (meaning.antonyms && meaning.antonyms.length > 0) {
            html += `<b>Antonyms:</b> ${meaning.antonyms.join(", ")}<br><br>`;
          }
          html += `<br><hr><br>`;
        });
