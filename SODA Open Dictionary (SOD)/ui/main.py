...
class Main(MDBoxLayout, TouchBehavior):
    ...
    def create_content_box(self, text, style=settings["homepage style"]):
        if style=="box":
            ...
            try:
                self.content_box=MDCard(orientation="vertical", md_bg_color=boxbg, padding=[10,10,10,10], size_hint=(None, None), width=300*scale, height=350*scale, pos_hint={"center_x":0.5, "center_y":0.5})
                self.content_box.radius=[i*scale for i in self.content_box.radius]
