...
class setting(MDBoxLayout):
    def __init__(self, **kwargs):
        ...
        self.noti_box=MDBoxLayout(size_hint=(1, None), height=50*scale, pos_hint={'center_x': 0.5, 'center_y': 0.5}, spacing=20*scale)
        self.noti_box.switch_box=MDBoxLayout(size_hint=(0.25, None), height=50*scale, pos_hint={'center_x': 0.5, 'center_y': 0.5}, spacing=20*scale)
        self.noti_box.time_value_box=MDBoxLayout(size_hint=(0.75, None), height=50*scale, pos_hint={'left': 0.9, 'center_y': 0.8}, spacing=20*scale)
        self.noti_box.title=MDLabel(text="Gợi ý từ mới qua thông báo", font_style="H6", size_hint=(1, None), height=30*scale, pos_hint={'center_x': 0.5, 'center_y': 0.5}, theme_text_color="Custom", text_color=primarycolor)
        self.noti_box.check=MDSwitch(icon_inactive="close", icon_active="check", thumb_color_inactive=btn, thumb_color_active=boxbg, track_color_inactive=boxbg, track_color_active=btn)
        self.noti_box.check.active=noti_require
        self.noti_box.check.bind(active=self.noti)
        self.noti_box.time_title=MDLabel(text="Thời gian chờ thông báo", font_style="Body2", halign="right", size_hint=(0.5, None), pos_hint={"right": 0.7}, height=30*scale, theme_text_color="Custom", text_color=primarycolor)        
        ...
        self.noti_box.switch_box.add_widget(self.noti_box.check)
        self.noti_box.add_widget(self.noti_box.time_value_box)
        self.noti_box.time_value_box.add_widget(self.noti_box.time_title)
        ...
        self.internet_required_check=MDSwitch(icon_inactive="close", icon_active="check", thumb_color_inactive=btn, thumb_color_active=boxbg, track_color_inactive=boxbg, track_color_active=btn)
        ...
        self.layout_scroll=ScrollView(do_scroll_y=False, size_hint=(1, None), height=125*scale)
        ...
        self.color_palette_scroll=ScrollView(do_scroll_y=False, size_hint=(1, None), height=300*scale)
        ...
        self.font_scroll=ScrollView(do_scroll_y=False, size_hint=(1, None), height=50*scale)
    ...
    def noti(self, instance, value):
        ...
        init.settings["notification"]=str(noti_require)
    ...
    def create_layout_preview(self, i):
        ...
        for j in range(2):
            box=MDCard(width=75*scale, height=22.5*scale, pos_hint={'center_x': 0.5}, md_bg_color=boxbg, ripple_behavior=True)
    ...
    def change_homepage_layout(self, instance, style):
        init.settings["homepage style"]=style
        self.dialog.open()
    ...
    def change_color_theme(self, instance, new_bg, new_boxbg, new_menubg, new_btn, new_primarycolor, new_secondarycolor):
        ...
        self.color_palette_scroll.scroll_to(self.got_check)
        init.settings["current palette"]="; ".join([", ".join([str(int(i*255)) for i in new_bg][:-1]),", ".join([str(int(i*255)) for i in new_boxbg][:-1]),", ".join([str(int(i*255)) for i in new_menubg][:-1]),", ".join([str(int(i*255)) for i in new_btn][:-1]), ", ".join([str(int(i*255)) for i in new_primarycolor][:-1]), ", ".join([str(int(i*255)) for i in new_secondarycolor][:-1])])
        self.dialog.open()
    ...
    def boost_performance(self, instance, value):
        init.settings["boost performance"]=not settings["boost performance"]
...
Nhật
        ...
        if self.timer is not None:
            self.timer.cancel()
        self.touch_count += 1
        if self.touch_count == 7:  # Số lần chạm xác định
