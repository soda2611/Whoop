...
def SOD_word_list():
    try:
        with open("tu_dien_nguon.txt", "r", encoding="utf-8") as f:
            lines = f.readlines()
    except:
        with open("func/tu_dien_nguon.txt", "r", encoding="utf-8") as f:
            lines = f.readlines()

    tu_dien = []
    for line in lines:
        try:
            word, wordtype, definition, translation, synonyms = line.strip().split(": ")
            tu_dien.append({"word": word, "type": wordtype, "definition": definition, "translation": translation})
        except:
            print(line)
    return tu_dien

def SOD(inp, internet_required=True, boost_performance=False):
    try:
        with open("tu_dien_nguon.txt", "r", encoding="utf-8") as f:
            lines = f.readlines()
    except:
    ...
    for line in lines:
        word, wordtype, definition, translation, synonyms = line.strip().split(": ")
        temp={"word": word, "type": wordtype, "definition": definition, "translation": translation}
        if synonyms!="none":
...
if __name__=="__main__":
    '''tudien=SOD_word_list()
    source=[]
    for i in tudien:
        source.append(i["word"])
    check='these evolutionary pressures helped mould keratin the substance from which breaks are made into a range of tools capable gathering new food types favored by various species parrot size short blunt beak and length that do curved upper section related to type each eats some have comparatively long beaks perfect for extracting seeds fruit others broader stronger designed cracking hard but other parrot species did not fare so well when their environments were altered the clearing of large areas rainforest is probably responsible for disappearance double-eyed fig and numbers ground parrots declined a great part habitat was destroyed by draining coastal swamps even some that benefited from forest at first are now comforted shortage nesting sites due to further man-made changes'.split()
    for i in check:
        if i not in source:
            print(i, end=" ")'''
    
    rewrite_word_source("func/word.txt")