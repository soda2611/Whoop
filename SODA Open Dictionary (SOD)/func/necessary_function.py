...
def check_connection(host="8.8.8.8", port=53, timeout=3):
    """
    Kiểm tra kết nối mạng bằng cách tạo một socket với Google DNS.
...
def spelling_checker_for_SOD(you):
    try:
        with open("word.txt",encoding="utf-8") as fi:
            word=[str(i) for i in fi.read().split()]
    except:
        with open("func/word.txt",encoding="utf-8") as fi:
            word=[str(i) for i in fi.read().split()]
