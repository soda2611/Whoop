...
import eng_to_ipa, os, random, threading , pyttsx3, subprocess, signal, winshell, getpass, sys, psutil, datetime
...
def config():
    if settings["uid"]=="none": settings["uid"]=datetime.datetime.now().strftime(f"%Y%m%d%H%M%S")
    engine = pyttsx3.init()
    data_=SOD_word_list()
