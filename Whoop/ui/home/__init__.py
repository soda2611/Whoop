...
class home(MDBoxLayout, TouchBehavior):
    ...
    def create_content_box(self, text):
        try:
            ...
            self.content_box.morebutton.bind(on_press=lambda instance: self.search_button_pressed(instance, text['type'], temp=text))
        except:
            pass
    ...
    def search_button_pressed(self, instance, input_text, temp=False):
        try:
            self.dialog.dismiss()
        except:
            pass
    ...
    def update_UI(self, instance):
        ...
        threading.Thread(target=track_user_queries).start()
        self.progress_bar.color=self.progress_bar.back_color
        self.progress_bar.stop()
